...
export default class extends PageModification {
  ...
  async renderWipLimitCloud() {
    ...
    for (const range of this.wip) {
      ...
      for(const cell of range.cells) {
        const issuesInCell = getIssueIdsBySwimlaneIdAndColumnId(
            boardLatest,
            cell.swimlane,
            cell.column,
        ).length;

        this.markCell(boardLatest, cell, range, issuesInCell);
      }
    }
  }

  getSwimlaneColumn(boardLatest, swimlaneId, columnId) {
    const swimlaneText = getSwimlaneById(boardLatest, swimlaneId)?.name;
    if (!swimlaneText) {
      return null;
    }
  ...
  markCell(boardLatest, cell, range, issuesInCell){
    const domElement = this.getSwimlaneColumn(boardLatest, cell.swimlane, cell.column);
